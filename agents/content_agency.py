...
if __name__ == "__main__":
    ...
    _demo_logger.info("CONTENT AGENCY - RAW CREATIVE DRAFT | Vault-Tec Pipeline: Stage 2")
    _demo_logger.info("=" * 80)
    _demo_logger.info("Draft ID: %s | Brief ID: %s | Title: %s", raw_draft.draft_id, raw_draft.research_brief_id, raw_draft.title)
    _demo_logger.info("Word Count: %s | Voice: %s | Tone: %s", raw_draft.overall_word_count, raw_draft.voice_used, raw_draft.tone_used)
    _demo_logger.info("Readability: %s | Consistency: %s | Engagement: %s | Flow: %s",
                      raw_draft.readability_scores.get("readability_score"), raw_draft.consistency_score,
                      raw_draft.engagement_score, raw_draft.flow_score)
    _demo_logger.info("Themes: %s | Arc: %s", ", ".join(raw_draft.key_themes), raw_draft.narrative_arc)
    _demo_logger.info("=" * 80)
    _demo_logger.info("HANDOFF PACKAGE FOR ON-PAGE SEO AGENCY")
    _demo_logger.info("=" * 80)
    _demo_logger.info(json.dumps(handoff, indent=2, default=str))
    _demo_logger.info("=" * 80)
